...
class ProductModeResultBot:
    ...
    def run(self):
        ...
        if is_production:
            # Webhook — для хостинга (Bothost, Render, Railway и т.д.)
            PORT = int(os.environ.get("PORT", 8443))
            webhook_base = os.getenv("WEBHOOK_URL", "https://your-bot.bothost.ru")  # ← УКАЖИ СВОЙ ДОМЕН/ПОДДОМЕН
            webhook_url = f"{webhook_base.rstrip('/')}/{self.token}"
            ...
            application.run_webhook(
                listen="0.0.0.0",
                port=PORT,
                url_path=self.token,               # Telegram будет слать на /{токен}
...
if __name__ == '__main__':
    ...
    if not token or "8483793056:AAECVHsX4yMTP4xWFdPmm_r2z6I7EGXMLD0" in token[:30]:
